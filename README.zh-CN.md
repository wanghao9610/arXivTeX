...
- **无外部字体依赖**:仅使用标准 TeX Live 字体,保证 GitHub 与 arXiv
  提交的可移植性。
- **一键转换**:使用 `convert-arxiv` 技能生成 [arXiv 预印本](#arxiv-pre-print);
...
- 一套 TeX Live 发行版(建议 2022 及以上版本),并确保 `latexmk` 在
  ...
  缺少宏包的报错。
- `make convert-arxiv` 需要 `latexpand` 和 `perl`(大多数 TeX Live 安装都自带)。
...
编辑完成后运行 `make convert-arxiv`,即可生成扁平化、可直接投稿的论文副本。
`convert-arxiv` 技能(同时为 Claude Code、Codex、Cursor 及其他智能体提供,
分别位于 `.claude/skills/convert-arxiv/`、`.codex/skills/convert-arxiv/`、
`.cursor/skills/convert-arxiv/` 和 `.agents/skills/convert-arxiv/`)对该流程做了
...
1. 按常规方式编辑 `main/` 下的论文,然后让你的智能体打包论文,例如:
   ...
   > 运行 make convert-arxiv 并确认输出能够编译通过。
2. 智能体会确认 `latexpand` 和 `perl` 已在 `PATH` 中,运行
   `make convert-arxiv`,并核对生成的 `arXiv/MANIFEST.txt` 与实际输出是否一致。
...
你也可以不借助智能体,自己直接运行 `make convert-arxiv`:
